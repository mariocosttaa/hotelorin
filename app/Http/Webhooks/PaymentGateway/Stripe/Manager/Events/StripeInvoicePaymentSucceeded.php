...
class StripeInvoicePaymentSucceeded
{
    ...
    {
        Log::info('Stripe Webhook Event - Stripe Invoice Payment Succeeded');

        try {
            // Acessar metadata do subscription_details ou lines->data[0]
            $metadata = (object) $event->data->object->subscription_details->metadata;
    ...
    private static function setInvoiceInSubscription(StripeEvent $event, object $metadata)
    {
        ...
        $subscription = SubscriptionModel::with('prices.currency')->where('id', $subscriptionId)->first();
        if (!$subscription) {
        ...
        if($metadata->set_first_invoice === 'true' || $metadata->set_first_invoice === true) {
            ...
            $invoice_local_id = EasyHashAction::decode($event->data->object->id, 'payment-gateway-invoice-id');
